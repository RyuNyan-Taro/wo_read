import 'package:wo_read/hiragana/models/example.dart';

class JapaneseData {
  static final List<String> hiraganaList = [
    // Basic Hiragana (あ行 to わ行)
    "あ", "い", "う", "え", "お", // あ行
    "か", "き", "く", "け", "こ", // か行
    "さ", "し", "す", "せ", "そ", // さ行
    "た", "ち", "つ", "て", "と", // た行
    "な", "に", "ぬ", "ね", "の", // な行
    "は", "ひ", "ふ", "へ", "ほ", // は行
    "ま", "み", "む", "め", "も", // ま行
    "や", "ゆ", "よ", // や行
    "ら", "り", "る", "れ", "ろ", // ら行
    "わ", "を", // わ行
    "ん", // ん
    // Dakuon (濁音)
    "が", "ぎ", "ぐ", "げ", "ご", // が行
    "ざ", "じ", "ず", "ぜ", "ぞ", // ざ行
    "だ", "ぢ", "づ", "で", "ど", // だ行
    "ば", "び", "ぶ", "べ", "ぼ", // ば行
    // Handakuon (半濁音)
    "ぱ", "ぴ", "ぷ", "ぺ", "ぽ", // ぱ行
  ];

  static final Map<int, Example> examples = {
    0: Example(reading: "あめ", imagePath: "assets/images/rain.png"),
    1: Example(reading: "いぬ", imagePath: "assets/images/dog.png"),
    2: Example(reading: "うさぎ", imagePath: "assets/images/rabbit.png"),
    3: Example(reading: "えんぴつ", imagePath: "assets/images/pen.png"),
    4: Example(reading: "おかし", imagePath: "assets/images/foods.png"),
    5: Example(reading: "かさ", imagePath: "assets/images/umbrella.png"),
    6: Example(reading: "きつね", imagePath: "assets/images/fox.png"),
    7: Example(reading: "くまくらいしさく", imagePath: "assets/images/bear.png"),
    8: Example(reading: "けーき", imagePath: "assets/images/cake.png"),
    9: Example(reading: "こおり", imagePath: "assets/images/ice.png"),
    10: Example(reading: "さく", imagePath: "assets/images/saku.jpeg"),
    11: Example(reading: "しか", imagePath: "assets/images/deer.png"),
    12: Example(reading: "すし", imagePath: "assets/images/sushi.png"),
    13: Example(reading: "せんべい", imagePath: "assets/images/senbei.png"),
    14: Example(reading: "そら", imagePath: "assets/images/sky.jpg"),
    15: Example(
      reading: "たこ",
      imagePath: "assets/images/charactor_takoyaki_tako.png",
    ),
    16: Example(
      reading: "ちず",
      imagePath: "assets/images/map_book_chizuchou.png",
    ),
    17: Example(reading: "つき", imagePath: "assets/images/moon.png"),
    18: Example(reading: "てぶくろ", imagePath: "assets/images/tebukuro.png"),
    19: Example(
      reading: "とり",
      imagePath: "assets/images/bird_aoitori_bluebird.png",
    ),
    20: Example(reading: "なす", imagePath: "assets/images/.png"),
    21: Example(
      reading: "にわとり",
      imagePath: "assets/images/bird_niwatori_chabo.png",
    ),
    22: Example(
      reading: "ぬいぐるみ",
      imagePath: "assets/images/toy_omocha_asobu_girl_oyako.png",
    ),
    23: Example(
      reading: "ねこ",
      imagePath: "assets/images/monogatari_alice_cheshire_neko.png",
    ),
    24: Example(reading: "のり", imagePath: "assets/images/food_nori.png"),
    25: Example(
      reading: "はな",
      imagePath: "assets/images/valentinesday_rose.png",
    ),
    26: Example(reading: "ひこうき", imagePath: "assets/images/airplane.png"),
    27: Example(
      reading: "ふね",
      imagePath: "assets/images/norimono_character2_fune.png",
    ),
    28: Example(reading: "へび", imagePath: "assets/images/hebi_blue.png"),
    29: Example(reading: "ほし", imagePath: "assets/images/christmas_star.png"),
    30: Example(reading: "まめ", imagePath: "assets/images/edamame.png"),
    31: Example(reading: "みかん", imagePath: "assets/images/fruit_orange2.png"),
    32: Example(
      reading: "むし",
      imagePath: "assets/images/bug_mark07_kanabun.png",
    ),
    33: Example(reading: "めがね", imagePath: "assets/images/megane_case.png"),
    34: Example(reading: "もも", imagePath: "assets/images/fruit_momo.png"),
    35: Example(reading: "やま", imagePath: "assets/images/bg_hatake.jpg"),
    36: Example(reading: "ゆき", imagePath: "assets/images/yuki_title.png"),
    37: Example(reading: "よる", imagePath: "assets/images/time4_yoru.png"),
    38: Example(
      reading: "らいおん",
      imagePath: "assets/images/animal_barbary_lion.png",
    ),
    39: Example(reading: "りんご", imagePath: "assets/images/ringo_shin.png"),
    40: Example(reading: "るびー", imagePath: "assets/images/yubiwa_ruby.png"),
    41: Example(
      reading: "れもん",
      imagePath: "assets/images/fruit_lemon_tategiri.png",
    ),
    42: Example(reading: "ろうそく", imagePath: "assets/images/rousoku_you.png"),
    43: Example(reading: "わに", imagePath: "assets/images/wani_open.png"),
    44: Example(reading: "をとこ", imagePath: "assets/images/man.png"),
    45: Example(reading: "えんぴつ", imagePath: "assets/images/big_pen.png"),
    46: Example(reading: "がっこう", imagePath: ""),
    47: Example(reading: "ぎんこう", imagePath: ""),
    48: Example(reading: "ぐんて", imagePath: ""),
    49: Example(reading: "げーむ", imagePath: ""),
    50: Example(reading: "ごはん", imagePath: ""),
    51: Example(reading: "ざっそう", imagePath: ""),
    52: Example(reading: "じてんしゃ", imagePath: ""),
    53: Example(reading: "ずぼん", imagePath: ""),
    54: Example(reading: "ぜんぶ", imagePath: ""),
    55: Example(reading: "ぞう", imagePath: ""),
    56: Example(reading: "だいこん", imagePath: ""),
    57: Example(reading: "ぢしん", imagePath: ""),
    58: Example(reading: "づくえ", imagePath: ""),
    59: Example(reading: "でんき", imagePath: ""),
    60: Example(reading: "どあ", imagePath: ""),
    61: Example(reading: "ばす", imagePath: ""),
    62: Example(reading: "びーる", imagePath: ""),
    63: Example(reading: "ぶた", imagePath: ""),
    64: Example(reading: "べんとう", imagePath: ""),
    65: Example(reading: "ぼうし", imagePath: ""),
    66: Example(reading: "ぱん", imagePath: ""),
    67: Example(reading: "ぴざ", imagePath: ""),
    68: Example(reading: "ぷーる", imagePath: ""),
    69: Example(reading: "ぺん", imagePath: ""),
    70: Example(reading: "ぽすと", imagePath: ""),
  };
}
